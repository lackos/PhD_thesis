...
\onehalfspacing
\title{PhD Thesis \\
Enhanced properties of deformed nuclei and spectra of Super Heavy Elements}
\begin{figure}
\center
...
Since its inception in the latter half of the of 20th century the Standard Model, which classifies and describes the interactions of known fundamental particles and forces, is a robust phenomenological theory and remains the most holistic and accurate description of our universe. However despite the apparent success of the SM \cite{} there are observed physical phenomena which cannot described without the SM being revised or extended. Similar to how classical physics is a large spatial approximation to quantum theory, it is suspected that the standard model is a low energy approximation to a more fundamental theory of the universe. To validate these beliefs and uncover new physics, scientists aim to discover physical phenomena not described by the current standard model, so-called ``beyond the standard model'' (BSM) physics. Many BSM theories have been proposed such as supersymmetric theories \cite{} One avenue of interest is the detection of the violation of fundamental symmetries is an avenue which is suspected to overthrow the model by suggesting a deeper underlying theory.  There are a number of discrete symmetries which form the basis of our current interpretation of the universe. They are time-reversal $T$ parity and charge conjugation (get the paragraph from Honours thesis). \\
...
Tests of Lorentz invariance date back to the famous Michelson and Morley experiment in the late 19th century %\cite{Michelson1887} 
which returned a null result (Confirming there is no lumeriferous ether and also verifying Lorentz symmetry). 
...
he study of super heavy elements (SHE), $Z > 100$, has been an important field in atomic and nuclear physics for the 
...
\chapter{Dubnium (Z=105)}
The ionisation potential is obtained by calculating the energy of the ground state of the ion and taking the difference 
between ground states of the ion and the neutral atom. The same single-electron basis is used for the ion as for the
neutral atom. 

\section{Ta I } \label{sec:TaI}
...
\multicolumn{4}{c}{Ta I} & \multicolumn{6}{c}{Db I} \\ \\
 & State &   \parbox{1cm}{$A_{E1}$ \\ (a.u)} & \parbox{1.5cm}{$T_{E1}$ \\ $(\times 10^{6} \ \text{s}^{-1})$  } & & State &   \parbox{2cm}{$A_{E1}$ \\ (a.u)} & \parbox{1.5cm}{$T_{E1}$ \\ $(\times 10^{6} \ \text{s}^{-1})$  } &  \parbox{1cm}{$a  $ (cm$^{-1}$)} & \parbox{1.5cm}{$F $ \\ (cm$^{-1}$/fm$^{2}$)} \\
\midrule
...
\begin{itemize}
\item $P$: A small set of low energy wavefunctions ($i \leq N_{\text{Eff}}$, where $N_{\text{Eff}}$ is the number of wavefunctions in the low energy set) that give dominant contributions to the CI wavefunction.
\item $Q$: A large set of high energy wavefunctions ($N_{\text{Eff}}<i \leq N_{\text{total}}$) that are corrections to the wavefunctions from $P$.
\end{itemize}
The CI wavefunction is written as an expansion over single-determinant many-electron states $|i \rangle $ from these two sets,
\begin{equation}
| \Psi \rangle = 
\sum_{i=1}^{N_{\text{Eff}}} c_{i}|i\rangle + \sum_{i = N_{\text{Eff}} + 1}^{N_{\text{total}}} c_{i}|i\rangle .
\label{eq:psi}
\end{equation}
where $c_i$ are coefficients of expansion. The CI Hamiltonian is truncated by neglecting the off-diagonal matrix elements of the CI Hamiltonian between terms in $Q$ ($\langle i | H^{\text{CI}} | j \rangle = 0 $ for $|i\rangle, |j\rangle \in Q$),  which reduces the problem of finding the wave function and corresponding energy to a matrix eigenvalue problem of the size $P$ with modified CI matrix
...
\sum_k \frac{\langle i|H^{\rm CI}|k\rangle\langle k|H^{\rm
    ...
    \label{eq:HCI}
\end{equation}
where $|i\rangle, |j\rangle \in P$, $|k\rangle \in Q$,  $E_k = \langle k|H^{\rm CI}|k\rangle$, and $E$ is the energy of the state of interest. 
...
\section{CIPT calculation of R\MakeLowercase{n} I and O\MakeLowercase{g} I} \label{sec:CIPT}
\begin{table*} [t!]
\begin{center}
...
\section{Electric dipole transitions of O\MakeLowercase{g} I} \label{sec:E1}
...
\section{Electron density of O\MakeLowercase{g} } \label{sec:Relativistic}
